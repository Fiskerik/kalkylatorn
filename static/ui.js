(function initUi(global) {
    ...
    const { beräknaNetto = () => 0 } = calculationUtils;
        ...
        group.forEach((button, index) => {
            ...
            button.addEventListener('click', () => {
                ...
                if (groupId === 'barn-tidigare-group') {
                    global.appState.barnIdag = parseInt(button.dataset.value, 10);
                }
                if (groupId === 'barn-planerade-group') {
                    global.appState.barnPlanerat = parseInt(button.dataset.value, 10);
                }
...
*/
    function generateParentSection(parentNum, dag, extra, månadsinkomst,
 ...
 */
    ...
    function updateMonthlyBox(wrapperId, dagarPerVecka, dag, extra, barnbidrag, tillägg, avtal, dagar) {
        ...
        if (!wrapper) {
            console.error(`Kunde inte hitta wrapper ${wrapperId}`);
            return;
        }

        const monthlyBox = wrapper.querySelector('.monthly-box');
        if (!monthlyBox) {
            console.error(`monthly-box hittades inte i ${wrapperId}`);
            return;
        }
